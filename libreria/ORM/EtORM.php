...
class EtORM extends \Conexion
{
    ...
    {
        $query = "DELETE FROM " . static::$table . " WHERE " . (is_null($columna) ? "id" : $columna) . " = ".$valor;
        //echo $query;
        self::getConexion();
        //preparar conexión
        $res = self::$cnx->prepare($query);
    ...
    public static function where2($columna, $valor,$comparador)
    {
        $query = "SELECT * FROM " . static::$table . " WHERE " . $columna .$comparador.":" . $columna;
        //echo $query;
        $class = get_called_class();
        self::getConexion();
        $res = self::$cnx->prepare($query);
    ...
    {
        $query = "SELECT 
        p.id,p.codigo,p.nombre,p.precio,p.minimo,p.estado,p.impuesto,p.descuento,m.medida,p.categoria,p.existencia,p.margen,1 as cantidad 
        FROM ". static::$table ." p INNER JOIN mediciones m 
        ON p.medida=m.id
        WHERE p.estado=1";
        //echo $query;
        $class = get_called_class();
